...
function Layout({ hasRightMargin = true, children }) {
    ...
    const data = myInfo.isLoading ? [] : myInfo.data;
    if (myInfo.isLoading) {
    ...
    return (
        ...
        <Container>
            <SideBar open={open} />
            <HideBtn active={open} onClick={() => setDrawerStatus(!open)}>
                <img src={open ? HideOn : HideOff} alt="" />
            </HideBtn>
            <ContentsContainer active={open}>
                <Header data={data} open={open} />
                {
                    !data.subscr_yn && !pathname.includes("my_account") && !pathname.includes("question") ? (
                    <Main active={open}>
                        <Subscr />
                    </Main>
                    ) : (
                    <Main active={open} hasRightMargin={hasRightMargin}>
                        {/* <div style={{ minWidth: "1080px", maxWidth: "1480px" }}>
                            {children}
                        </div> */}
                        <MainWrap active={open}>
                            {children}
                        </MainWrap>
                    </Main>
                    )
                }
                {/* <Main hasRightMargin={hasRightMargin}>{children}</Main> */}
            </ContentsContainer>
        </Container>
        </>
    );
...
const MainWrap = styled.div`
    ...
    @media (min-width: 1440px) and (max-width: 1919px) {          
        min-width: ${(props) => (props.active ? "1380px" : "900px")};
    }
    @media (min-width: 10px) and (max-width: 1439px) {
