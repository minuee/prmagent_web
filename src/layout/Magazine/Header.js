...
export default function Header({open}) {
  ...
  const handleUserSetting = () => {
    history.push("/magazine/my_account");
  };
  ...
  const handleSubmit = () => {
    if (searchRef.current.value.length < 2) {
      ...
      history.push(`/magazine/search/${searchRef.current.value.toString()}`, {
  ...
  const handleAlarm = () => {
    history.push("/magazine/alarm");
  };
  const handleFavorites = () => {
    history.push("/magazine/favorites");
  };
        ...
        <HeaderDataContainer>
          ...
          <RightMenuWrap>
            <FavoriteWrap onClick={handleFavorites}>
              {location.pathname.startsWith("/magazine/favorites") ? (
                <FavoritesOnIcon style={{maxHeight:50}} />
              ) : (
                <FavoritesOffIcon style={{maxHeight:50}} />
              )}
            </FavoriteWrap>
            <AlarmWrap onClick={handleAlarm}>
              <img
                src={
                  location.pathname.startsWith("/magazine/alarm")
                    ? AlarmOnIcon
                    : AlarmOffIcon
                }
                style={{maxHeight:50}}
            ...
            <UserAvatar onClick={() => handleToggleClick(!viewOpen)}>
              <Avatar imgUrl={profile_img} style={{maxHeight:50}} />
              {viewOpen && (
                <Popup>
                  <PopupMenus
              ...
              <UserWrap>
                <UserSettingWrap>
                  {data.mgzn_user_nm} {data.user_position}
                </UserSettingWrap>
