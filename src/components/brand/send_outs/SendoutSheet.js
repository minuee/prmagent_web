...
function NoticeComponent() {
  ...
  const handleDelete = useCallback(() => {
   /*  if (confirm("연결시트 공지사항을 삭제하시겠습니까?")) {
      setNotice("");
      noticeUpdate.mutate(notice);
    } */
    alertConfirm({
      title: Constants.appName,
      content: '연결시트 공지사항을 삭제하시겠습니까?',
      onOk: () => {
        setNotice("");
        noticeUpdate.mutate(notice);
      },
  ...
  const handleConfirm = useCallback(() => {
    /* if (confirm("연결시트 공지사항을 수정하시겠습니까?")) {
      noticeUpdate.mutate(notice);
    } */
    alertConfirm({
      title: Constants.appName,
      content: '연결시트 공지사항을 수정하시겠습니까?',
      onOk: () => {
        noticeUpdate.mutate(notice);
      },
      onCancel: () => {console.log('cancel')}
  ...
  const noticeUpdate = useMutation(
    ...
    {
      onSuccess: () => {
        // utils.customAlert("연결시트 공지사항이 수정되었습니다.");
        queryClient.invalidateQueries(["sendout-notice"]);
      },
      onError: () => {
        utils.customAlert("연결시트 공지사항 수정 중 오류가 발생했습니다.");
      },
    }
  );
  ...
  return (
    <>
      <Container>
        <Title>연결시트 공지사항</Title>
        <StyleDivider />
        <Contents>
          <ContentsTitle>
            연결시트에 들어갈 공지사항을 기재해주세요.
          </ContentsTitle>
          <StyledTextField
            multiline
            rows={14}
