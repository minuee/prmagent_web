...
function ReqBoard({data,memo,dt,h,setH,idx,wait,endDt,season,gender,look}) {
  const week = [
    moment(dt).weekday(0),
    moment(dt).weekday(1),
    moment(dt).weekday(2),
      ...
      <Container height={height}>
        <FlexBox>
          {week.map((n, index) => (            
            <Wrap style={{ display: "flex", flexDirection: "column" }} key={n}>
              <DailyMemo data={n} idx={index} look={look} inputs={inputs} />
            </Wrap>
          )
          )}
        </FlexBox>
          ...
          data.map((d) => {
            ...
            return (
              <>
                ...
                <ReqWrap key={d.req_no} width={d.from} height={chk[d.from]} inputs_len={inputs}>
        ...
        {wait !== null &&
          _.uniqBy(wait, "from").map((d) => {
            chk[d.from] = chk[d.from] + 1;

            return (
              <ReqWaitWrap width={d.from} height={chk[d.from]} key={d.req_no}>
                <ReqWait
                  data={wait}
                  idx={d.from}
                  startDt={dt}
...
const Container = styled.div`
  display: flex;
  height: ${(props) =>
    props.height <= 2 ? "550px" : (props.height * 220) + 148 + "px"};
  max-width:1420px;
  position: relative;
...
const ReqWrap = styled.div`
  ...
  margin-bottom:20px;
  top: ${(props) =>
    props.height === 1 ? "0px" : 210 * (props.height - 1) + "px"};
  left: ${(props) =>
    props.width === 0
      ? "10px"
      : props.width === 1
...
const Req = styled.div`
  ...
  .head {
    ...
    width: ${(props) =>
      props.leng === 1
        ? "180px"
        : props.leng * 180 + (props.leng - 1) * 20 + "px"};
