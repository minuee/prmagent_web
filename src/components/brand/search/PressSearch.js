...
function PressSearch({ searchText, total, setTotal }) {
  const history = useHistory();
  const [cnt, setCnt] = useState(0);
  const getSearchList = ({ pageParam = 1 }) =>
    apiObject.getPressSearch({ page: pageParam, search_text: searchText });

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } =
    useInfiniteQuery(["main-search", "press", searchText], getSearchList, {
    ...
    if (status !== "loading") {
      data.pages.map((d) => {
        if (d.press.length === 0) {
          ...
          setCnt(parseInt(d.press[0].total_count));
          setTotal({ ...total, press: parseInt(d.press[0].total_count) });
        }
      });
    }
  }, [data, status]);
                  ...
                  <ItemWrap active={isdrawer}>
                    {group.press.map((d) => (
                      <Item
                        ...
                        onClick={() =>
                          history.push(
                            `/brand/press_release/detail/${d.brand_press_no}`
                          )
                        }
                        active={isdrawer}
                      >
                        ...
                        <TextWrap active={isdrawer}>
                          <div className="title">{d.press_subj}</div>
                          <div className="season">
                            {dayjs(d.input_dt).format("YYYY-MM-DD")}
                          </div>
                        </TextWrap>
                      </Item>
                    ))}
...
export default React.memo(PressSearch);
