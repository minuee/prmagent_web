...
function SendoutSearch({ searchText, total, setTotal }) {
  const history = useHistory();
  const [cnt, setCnt] = useState(0);
  const getSearchList = ({ pageParam = 1 }) =>
    apiObject.getSendoutSearch({ page: pageParam, search_text: searchText });

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } =
    useInfiniteQuery(["main-search", "sendout", searchText], getSearchList, {
  ...
  useEffect(() => {
    if (status !== "loading") {
      data.pages.map((d) => {
        if (d.sendout.length === 0) {
          setCnt(0);
        } else {
          setCnt(parseInt(d.sendout[0].total_count));
          setTotal({ ...total, sendout: parseInt(d.sendout[0].total_count) });
        }
      });
    }
  }, [data, status]);
    ...
    <Container>
      ...
      {status === "loading" ? (
        ...
        <>
          {data.pages.map(
            (group, idx) =>
              group.sendout.length !== 0 && (
                <React.Fragment key={idx}>
                  <ItemWrap active={isdrawer}>
                    {group.sendout.map((d, i) => (
                      <Item
                        key={`${i}_items`}
                        onClick={() =>
                          history.push(`/brand/send_outs/${d.recpt_dt}`)
                        }
                        active={isdrawer}
                      >
                        <Img
                          imgUrl={d.img_url_adres}
                          bg={d.mgzn_color || "#e7e7e7"}
                        />
...
export default React.memo(SendoutSearch);
