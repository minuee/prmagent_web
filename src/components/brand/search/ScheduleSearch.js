...
function ScheduleSearch({ searchText, total, setTotal }) {
  const history = useHistory();
  const [cnt, setCnt] = useState(0);
  const getSearchList = ({ pageParam = 1 }) =>
    apiObject.getScheduleSearch({ page: pageParam, search_text: searchText });

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } =
    useInfiniteQuery(["main-search", "schedule", searchText], getSearchList, {
    ...
    if (status !== "loading") {
      data.pages.map((d) => {
        if (d.schedule.length === 0) {
          ...
          setCnt(parseInt(d.schedule[0].total_count));
          setTotal({ ...total, schedule: parseInt(d.schedule[0].total_count) });
        }
      });
    }
  }, [data, status]);
              ...
              group.schedule.length !== 0 && (
                <React.Fragment key={idx}>
                  <ItemWrap active={isdrawer}>
                    {group.schedule.map((d, i) => (
                      <Item
                        key={`${i}_items`}
                        onClick={() =>
                          history.push(`/brand/scheduler/${d.recpt_dt}`)
                        }
                        active={isdrawer}
                      >
...
export default React.memo(ScheduleSearch);
