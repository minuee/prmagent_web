...
function LookbookSearch({ searchText, total, setTotal }) {
  const history = useHistory();
  const [cnt, setCnt] = useState(0);
  const getSearchList = ({ pageParam = 1 }) =>
    apiObject.getLookbookSearch({ page: pageParam, search_text: searchText });

  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, status } =
    useInfiniteQuery(["main-search", "lookbook", searchText], getSearchList, {
  ...
  useEffect(() => {
    if (status !== "loading") {
      data.pages.map((d) => {
        if (d.lookbook.length === 0) {
          setCnt(0);
        } else {
          setCnt(parseInt(d.lookbook[0].total_count));
          setTotal({ ...total, lookbook: parseInt(d.lookbook[0].total_count) });
        }
      });
    }
  }, [data, status]);
    ...
    <Container>
      ...
      {status === "loading" ? (
        ...
        <>
          {data.pages.map(
            (group, idx) =>
              group.lookbook.length !== 0 && (
                <React.Fragment key={idx}>
                  <ItemWrap active={isdrawer}>
                    {group.lookbook.map((d) => (
                      <Item
                        key={d.lookbook_no}
                        onClick={() =>
                          history.push(
                            `/brand/lookbook/detail/${d.lookbook_no}`
                          )
                        }
                        active={isdrawer}
                      >
                        ...
                        <TextWrap active={isdrawer}>
                          <div className="title">{d.lookbook_nm}</div>
                          <div className="season">
                            {dayjs.unix(d.lookbook_reg_dt).format("YYYY-MM-DD")}
                          </div>
                        </TextWrap>
                      </Item>
                    ))}
...
export default React.memo(LookbookSearch);
