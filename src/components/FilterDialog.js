...
import {DialogTitle,DialogContent,Dialog,DialogActions,Divider} from "@material-ui/core";
...
import CheckboxComponent from "./brand/digital_showroom/CheckboxComponent";
...
import { 
  currentfilterSubmenu,
  currentSelectGender,
  currentSelectCategory,
  currentCheckCategoryAll,
  currentSelectMaterial,
  currentCheckMaterialAll,
  currentSelectColor,
  currentCheckColorAll,
  currentSelectSize,
  currentCheckSizeAll
} from "redux/state";


function FindIdDialog({ open, setOpen, filterData, setFilterData }) {
  ...
  const [filterSubmenu, setFilterSubmenu] = useRecoilState(currentfilterSubmenu);
  const [selectGender, setSelectGender] = useRecoilState(currentSelectGender);
  const [selectCategory, setSelectCategory] = useRecoilState(currentSelectCategory);
  const [checkCategoryAll, setCheckCategoryAll] = useRecoilState(currentCheckCategoryAll);
  const [selectMaterial, setSelectMaterial] = useRecoilState(currentSelectMaterial);
  const [checkMaterialAll, setCheckMaterialAll] = useRecoilState(currentCheckMaterialAll);
  const [selectColor, setSelectColor] = useRecoilState(currentSelectColor);
  const [checkColorAll, setCheckColorAll] = useRecoilState(currentCheckColorAll);
  const [selectSize, setSelectSize] = useRecoilState(currentSelectSize);
  const [checkSizeAll, setCheckSizeAll] = useRecoilState(currentCheckSizeAll);
  ...
  const handleFilterSubmenu = useCallback(
    (data) => {
  ...
  const handleMaterialSelectAll = useCallback(
    (data) => {
      if (!checkMaterialAll) {
        let arr = [];
    ...
    [checkMaterialAll, selectMaterial]
  );
  ...
  const handleColorSelectAll = useCallback(
    (data) => {
      if (!checkColorAll) {
        let arr = [];
        data.forEach((d) => arr.push(d.cd_id));
  ...
  const sampleData = useMemo(
    () => (sampleInfo.isLoading ? [] : sampleInfo.data),
    [sampleInfo.data]
  );
