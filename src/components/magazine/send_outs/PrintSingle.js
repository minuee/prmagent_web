  ...
  useEffect(async() => {
    if ( !utils.isEmpty(data)) {
      if ( view === 'pickups') {
        ...
        const sendout_date = dayjs.unix(data.sendout_date).format("YYYY-MM-DD");
        if ( !utils.isEmpty(data.showroom_list) ) {
          data.showroom_list.forEach((item1,i1) => {
            if ( !utils.isEmpty(item1.sample_list) ) {
              item1.sample_list.forEach((item2,i2) => {
                if ( utils.dateToDate(item2.sendout_dt) !== sendout_date ) {
                  setChangeReturnDate(utils.dateToDate(item2.sendout_dt));
                }
              })
            }
          }
      ...
      <Content>
        <Table>
          ...
          <tbody>
                ...
                d.sample_list.map((v, i) => (
                  <React.Fragment key={v.sample_no}>
                    <TBodyTr>
                      ...
                      <TbodyTd>{v.category}</TbodyTd>
                        ...
                        <TbodyTdWrap bg="#d78979">                          
                          ...
                          { dayjs.unix(data.sendout_date).format("YYYY-MM-DD") !== utils.dateToDate(v.sendout_dt) &&
                          <><br />발송일 ({utils.dateToDate(v.sendout_dt)})</>
                          }                          
                        </TbodyTdWrap>
                        <TbodyTd bg="#e1c668">
                          ...
                          {/* ({utils.isEmpty(v.use_user_info) ? data.brand_nm : v.use_user_info?.[0].mgzn_nm}) */} 
                        </TbodyTd>
                    ...
                    <TBodyTr>
                      ...
                      <TbodyTd>
                        {utils.isEmpty(v.use_user_info) ? utils.phoneFormat(data.to_user_phone) : utils.phoneFormat(v.use_user_info?.[0].phone_no)}
                      </TbodyTd>
                    </TBodyTr>
            ...
            <tr>
              <td
                style={{ fontWeight: "bold",fontSize: "12px",paddingTop: "15px",}}
                colSpan={5}
              >
                {footer}
              </td>
