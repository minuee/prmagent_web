...
export default function AddComponent({data,brandId,handleCancel,handleSubmit,handleTemporarySave,}) {    
    ...
    const [inputs, setInputs] = useState({
        ...
        dlvy_atent_matter: "",
        contact_user_id: "",
        //req_user_id: myinfo.length > 0 ? myinfo[0].user_id : "",
        req_user_id :  !utils.isEmpty(myUserDataLocal) ? myUserDataLocal.username : null,
        loc_yn: false,
        loc_value: "",
    });
    ...
    const [endTime, setEndTime] = useState({hour: "11",ampm: "PM",});
    ...
    data.showroom_info.forEach((d2, i2) => {
        if ( !utils.isEmpty(d2.reservation_list) ) {
            d2.reservation_list.forEach((d3, i3) => {
                reservationArray = reservationArray.concat(d3.photogrf_dt);
            })
        }
    }); */
