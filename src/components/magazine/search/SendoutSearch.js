...
export default function SendoutSearch({ searchText, total, setTotal }) {
  ...
  const {
    ...
    ["main-search-mgzn", "sendout", searchText],
    getSearchList,
    {
    ...
    if (status !== "loading") {
      data.pages.map((d) => {
        if (d.returns.length === 0) {
          ...
          setCnt(parseInt(d.returns[0].total_count));
          setTotal({ ...total, sendout: parseInt(d.returns[0].total_count) });
        }
      });
    }
  }, [data, status]);
              ...
              group.returns.length !== 0 && (
                <React.Fragment key={idx}>
                  <ItemWrap active={isdrawer}>
                    {group.returns.map((d, i) => (
                      <Item
                        key={`${i}_items`}
                        onClick={() => history.push(`/magazine/pickup`)}
                        active={isdrawer}
                      >
                        ...
                        <TextWrap active={isdrawer}>
                          ...
                          <div className="season">
                            {dayjs.unix(d.return_dt).format("YYYY-MM-DD")}
