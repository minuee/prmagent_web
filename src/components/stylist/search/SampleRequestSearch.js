...
export default function SampleRequestSearch({ searchText, total, setTotal }) {
  const history = useHistory();
  const [cnt, setCnt] = useState(0);
  const getSearchList = ({ pageParam = 1 }) =>
    apiObject.getReqSearch({ page: pageParam, search_text: searchText });
  ...
  const {
    ...
    ["main-search-stylist", "sample-request", searchText],
    getSearchList,
    {
  ...
  useEffect(() => {
    if (status !== "loading") {
      data.pages.map((d) => {
        if (d.request.length === 0) {
          setCnt(0);
        } else {
          setCnt(parseInt(d.request[0].total_count));
          setTotal({ ...total, request: parseInt(d.request[0].total_count) });
        }
      });
    }
  }, [data, status]);
  ...
  return (
    <Container>
      <Title>Sample Request ({cnt})</Title>
      {status === "loading" ? (
        <Progress type="load" />
      ) : (
        <>
              ...
              group.request.length !== 0 && (
                <React.Fragment key={idx}>
                  <ItemWrap active={isdrawer}>
                    {group.request.map((d, i) => (
                      <Item
                        key={`${d.req_no}_${i}`}
                        onClick={() =>
                          history.push(
                            `/stylist/sample_requests/detail/${d.req_no}`
                          )
                        }
                        active={isdrawer}
                      >
