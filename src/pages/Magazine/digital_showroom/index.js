...
import DigitalShowroomTitle from "components/magazine/digital_showroom/DigitalShowroomTitle";
import DigitalShowroomItems from "components/magazine/digital_showroom/DigitalShowroomItems";
...
export default function DigitalShowroom({ match }) {
    ...
    const BRAND_ID = match.params.brand_id === undefined ? 'all' : match.params.brand_id;
    ...
    const handleSelectData = (no) => {
        //console.log('handleSelectData',no)
        if ( no.all_in_yn ) {
            setSelectData(selectData.includes(no)? selectData.filter((d) => d !== no): [...selectData, no]);
        }
    };
    ...
    const handleCreateRequestSample = () => {
        ...
        if (imsi !== null && imsi.brand_id === current_brand_info.brand_id) {
            ...
            let param = "";
            selectData.map((d, i) => i === 0 ? (param = d.showroom_no) : (param = param + "+" + d.showroom_no));
            history.push("/magazine/sample_requests/add/" + current_brand_info.brand_id +"=" +param);
        }
    };
    ...
    const handleMoveTemporarySave = () => {
        let imsi = JSON.parse(localStorage.getItem("imsi"));
        let showroomList = "";
        imsi.showroom_list.map((d, i) => i === 0 ? (showroomList = d) : (showroomList = showroomList + "+" + d));
        history.push("/magazine/sample_requests/temporary/" +current_brand_info.brand_id +"=" +showroomList);
    };
    ...
    useIntersectionObserver({target: loadMoreButtonRef,onIntersect: fetchNextPage,enabled: hasNextPage,});

    const season_list = status === "loading" ? []: data?.pages[0]?.season_list.map((item) => ({
        season_year: item.season_year,
        season_cd_id: item.season_cd_id,
        label: item.season_year + " " + item.season_text,
    }));

    const current_brand_info =status === "loading" ? [] : data.pages[0].current_brand_info;
    const brand_list = status === "loading" ? [] : data.pages[0].brand_list;
    const brand_notice = status === "loading" ? [] : data.pages[0].brand_notice;

    const setFavShowroom = useMutation(["fav-set-showroom"],(value) =>
        apiObject.setFavShowroom({showroom_no: value.showroom_no,}),
        {
            onSuccess: () => {
                refetch({ refetchPage: (page, index) => index === 0 });
            },
            onError: () => {
                utils.customAlert("쇼룸 즐겨찾기 등록중 오류가 발생했습니다.");
            },
        }
    );

    const delFavShowroom = useMutation(["fav-del-showroom"],(value) =>
        ...
        <>
            ...
            <ButtonDiv>
                <div ref={loadMoreButtonRef} onClick={() => fetchNextPage()}>
                    {isFetchingNextPage ? ( <CircularProgress /> ) : hasNextPage ? ( "더보기") : ("")}
                </div>
            </ButtonDiv>
            <FilterDialog
...
const ContensWrap = styled.div`
    ...
    @media (min-width: 10px) and (max-width: 1439px) {
        min-width: ${(props) => (props.active ? "900px" : "650px")};    
    } 
`;
