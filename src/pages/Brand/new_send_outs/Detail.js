...
export default function SendOutDetail({ match }) {
  ...
  const handleItemClick = (data,idx) => {
    const nowData = loadData.left.each_list[idx].showroom_list[0].req_no;
    setPrintDataIdx(idx);
    setPrintData(nowData);
    /* console.log('111111',loadData.left.each_list[printData].showroom_list[0].req_no)
    console.log('22222',loadData.left.each_list[idx].showroom_list[0].req_no)
    if ( printData !== idx) {
      if ( loadData.left.each_list[printData].showroom_list[0].req_no != loadData.left.each_list[idx].showroom_list[0].req_no) {
        setPrintData(idx);
      }
    }*/
  };
  ...
  useEffect(async() => {
    ...
    if (timeStamp && paramData.length > 0 ) {
      ...
      console.log('queryData',queryData.left)
      let newLeftIdxArray = [];
      let newLeftArray = [];
  ...
  //console.log('data.left.each_list',printData)
